...
public class LoginActivity extends AppCompatActivity {
    ...
    private void attemptLogin() {
       /* if (mAuthTask != null) {
        ...
        if (cancel) {
            Log.w(TAG, "login validation failed for user=[" + userName + "]");
            // There was an error; don't attempt login and focus the first
            // form field with an error.
            focusView.requestFocus();
        } else {
            Log.d(TAG, "login attempt started for user=[" + userName + "]");
    ...
    void login() {
        ...
        if (URL != null && URL.contains("xmwgw")) {
            Log.d(TAG, "login -> legacy path user=[" + user + "] url=[" + URL + "]");
            loginLegacy(user, pass);
            return;
        }

        Log.d(TAG, "login -> RFC path user=[" + user + "] rfc=[" + Vars.ZWM_USER_AUTHORITY_CHECK + "] url=[" + URL + "]");
        ...
        {   Log.e(TAG, "login -> failed to build request", e);
            dialog.dismiss();
            box.getErrBox(e);
        }
    }

    /** Log login RFC payload without exposing the password. */
    private String loginArgsForLog(JSONObject args) {
        try {
            JSONObject copy = new JSONObject(args.toString());
            if (copy.has("IM_PASSWORD")) {
                copy.put("IM_PASSWORD", "***");
            }
    ...
    private void loginLegacy(String user, String pass) {
        ...
        Log.d(TAG, "loginLegacy request -> url=[" + URL + "] user=[" + user + "]");
    ...
    private void handleLegacyLoginResponse(String response, String user, String pass) {
        if (response == null || response.trim().isEmpty()) {
            Log.w(TAG, "loginLegacy response -> empty for user=[" + user + "]");
        ...
        if (parts.length >= 1 && parts[0].trim().equals("1")) {
            ...
            Log.d(TAG, "loginLegacy response -> success user=[" + user + "] WERKS=[" + werks + "]");
            ...
            Log.w(TAG, "loginLegacy response -> failed user=[" + user + "] raw=[" + response + "]");
    ...
    private void fetchUserGroupAndRoute(String user, String pass,
        ...
        try {
            ...
            args.put("IM_PASSWORD", pass);
            Log.d(TAG, "loginLegacy authority check -> rfc=[" + Vars.ZWM_USER_AUTHORITY_CHECK + "]");
            Log.d(TAG, "loginLegacy authority check -> url=[" + rfcUrl + "]");
            Log.d(TAG, "loginLegacy authority check -> payload=" + loginArgsForLog(args));
        } catch (JSONException e) {
            Log.e(TAG, "loginLegacy authority check -> failed to build request", e);
            e.printStackTrace();
        ...
        com.android.volley.toolbox.JsonObjectRequest req =
            new com.android.volley.toolbox.JsonObjectRequest(
                ...
                responsebody -> {
                    Log.d(TAG, "loginLegacy authority check -> response=" + responsebody);
                    String group = "";
                    try {
                        ...
                        Log.e(TAG, "loginLegacy authority check -> parse error", e);
                        e.printStackTrace();
                    }
                    Log.d(TAG, "loginLegacy authority check -> EX_GROUP=[" + group + "] EX_WERKS=[" + data.read("WERKS") + "]");
                    routeUser(group, data.read("WERKS"), data);
                },
                error -> {
                    Log.e(TAG, "loginLegacy authority check -> error", error);
    ...
    private void submitRequest(String rfc, int request, JSONObject args){
        ...
        Log.d(TAG, "login RFC request -> " + rfc);
        Log.d(TAG, "login RFC url -> " + url);
        Log.d(TAG, "login RFC payload -> " + loginArgsForLog(params));
        ...
        mJsonRequest = new SapJsonObjectRequest(Request.Method.POST, url, params, new Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject responsebody) {
                ...
                Log.d(TAG, "login RFC response -> " + responsebody);

                if (responsebody == null) {
                    Log.w(TAG, "login RFC response -> null");
                    box.getBox("Err", "No response from Server");
                } else if (responsebody.equals("") || responsebody.equals("null") || responsebody.equals("{}")) {
                    Log.w(TAG, "login RFC response -> empty");
                    ...
                    try {
                        if (responsebody.has("EX_RETURN") && responsebody.get("EX_RETURN") instanceof JSONObject) {
                            ...
                            if (returnobj != null) {
                                ...
                                if (type != null) {
                                    if (type.equals("E")) {
                                        Log.w(TAG, "login RFC response -> error TYPE=E message=[" + returnobj.optString("MESSAGE") + "]");
                        ...
                        Log.e(TAG, "login RFC response -> parse error", e);
                        e.printStackTrace();
                        box.getErrBox(e);
                    }
                }
    ...
    Response.ErrorListener volleyErrorListener() {
        return new Response.ErrorListener() {
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                Log.e(TAG, "login RFC error -> " + err, error);
