 ...
 *       After save: clear BIN+Palette, keep PO/Vendor/Inv.
 * @version 12.106
 */
public class FragmentInboundPutwayToBin extends Fragment implements View.OnClickListener {

    private static final String TAG          = "InboundPutwayToBin";
    private static final String RFC_BIN_VAL  = "ZVND_PUTWAY_BIN_VAL_RFC";
    private static final String RFC_PALL_VAL = "ZVND_PUTWAY_PALETTE_VAL_RFC";
    ...
    private String validatedBin = "", validatedPall = "";
    private String poNo = "", billNo = "", vendorName = "";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        etBin.setOnEditorActionListener((v,a,e) -> { String b=etBin.getText().toString().trim(); if(!b.isEmpty()) validateBin(b); return true; });
        etPalette.setOnEditorActionListener((v,a,e) -> { if(!binValidated){showStatus("Scan BIN first.",false);return true;} String p=etPalette.getText().toString().trim(); if(!p.isEmpty()) validatePalette(p); return true; });
        init(); return view;
    }
    ...
    private void validateBin(String bin) {
        ...
        try{p.put("bapiname",RFC_BIN_VAL);p.put("IM_USER",USER);p.put("IM_PLANT",WERKS);p.put("IM_BIN",bin);}
        catch(JSONException e){dismissProgress();return;}
        callRfc(RFC_BIN_VAL,p,r->{
    ...
    private void validatePalette(String palette) {
        ...
        JSONObject p=new JSONObject();
        try{p.put("bapiname",RFC_PALL_VAL);p.put("IM_USER",USER);p.put("IM_PLANT",WERKS);p.put("IM_BIN",validatedBin);p.put("IM_PALL",palette);}
        catch(JSONException e){dismissProgress();return;}
        callRfc(RFC_PALL_VAL,p,r->{
            try{ if(isSuccess(r)){
                ...
                if(et!=null&&et.length()>0){
                    ...
                    tvBoxCount.setText("Boxes: "+boxes);tvBoxCount.setVisibility(View.VISIBLE);
                }
    ...
    private void save() {
        ...
        try{
            ...
            JSONArray it=new JSONArray();it.put(row);
            p.put("IT_DATA",it);
        }catch(JSONException e){dismissProgress();return;}
        callRfc(RFC_SAVE,p,r->{
            try{if(isSuccess(r)){showStatus("✔ Saved! Palette "+validatedPall+" → BIN "+validatedBin,true);partialReset();}
            else{showStatus("Save Error: "+getMsg(r),false);}}
            catch(JSONException e){showStatus("Parse error",false);}
        },err->showStatus("Network: "+err,false));
    }

    // After save: clear only BIN + Palette. Keep PO/Vendor/Inv.
    private void partialReset() {
        binValidated=false;pallValidated=false;validatedBin="";validatedPall="";
        etBin.setText("");etPalette.setText(""); etBin.setEnabled(true);etPalette.setEnabled(false);btnSave.setEnabled(false);
        etBin.requestFocus();
        showStatus("BIN & Palette cleared. Scan next BIN.",true);
    }

    private void fullReset() {
        partialReset();poNo="";billNo="";vendorName="";
        tvPo.setVisibility(View.GONE);tvInv.setVisibility(View.GONE);tvVendor.setVisibility(View.GONE);tvBoxCount.setVisibility(View.GONE);
        showStatus("Scan Destination BIN.",true);
    }
