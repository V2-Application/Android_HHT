...
public class TSPLPrinter {
    ...
    /**
     * Print HU swap label after {@link com.v2retail.commons.Vars#ZWM_HUSWAP} returns new HU data.
     */
    public void sendHuSwapPrintCommand(String printerName, String oldHu, String newHu,
            String vemng, String crOn, String crAt, String source, String newDes) {
        try {
            findBluetoothPrinter(printerName, false);
            if (printerDevice == null) {
                Log.e("TSPLPrinter", "Bluetooth printer not found: " + printerName);
                return;
            }
            connectToBluetoothPrinter();
            String tspl = buildHuSwapTspl(oldHu, newHu, vemng, crOn, crAt, source, newDes);
            if (bluetoothSocket != null && bluetoothSocket.isConnected()) {
                OutputStream outputStream = bluetoothSocket.getOutputStream();
                PrintWriter writer = new PrintWriter(outputStream, true);
                writer.write(tspl);
    ...
    private static String tsplQuote(String s) {
        if (s == null) {
            return "";
        }
    ...
    private String buildHuSwapTspl(String oldHu, String newHu, String vemng,
            String crOn, String crAt, String source, String newDes) {
        oldHu = tsplQuote(oldHu);
        newHu = tsplQuote(newHu);
        vemng = tsplQuote(vemng);
        crOn = tsplQuote(crOn);
        crAt = tsplQuote(crAt);
        source = tsplQuote(source);
        newDes = tsplQuote(newDes);
        return "SIZE 70 mm, 40 mm\n"
                + "GAP 3 mm, 0 mm\n"
                + "DIRECTION 0\n"
                + "CLS\n"
                + "TEXT 20, 30, \"2\", 0, 1, 1, \"HU SWAP\"\n"
                + "TEXT 20, 55, \"2\", 0, 1, 1, \"Old: " + oldHu + "\"\n"
                + "TEXT 20, 80, \"2\", 0, 1, 1, \"New: " + newHu + "\"\n"
                + "TEXT 20, 105, \"2\", 0, 1, 1, \"Qty: " + vemng + "\"\n"
                + "TEXT 20, 130, \"2\", 0, 1, 1, \"" + crOn + "  " + crAt + "\"\n"
                + "TEXT 20, 155, \"2\", 0, 1, 1, \"Src: " + source + "\"\n"
                + "TEXT 20, 180, \"2\", 0, 1, 1, \"" + newDes + "\"\n"
                + "BARCODE 80, 220, \"128\", 70, 0, 0, 2, 4, \"" + newHu + "\"\n"
                + "PRINT 1,1\n";
    }
